...
-- Xác nhận rớt lần hai.
--
-- Bản trước QĐ này đặt số mới = 10.000, mất cả phần khoa sửa lẫn phần rớt lần
-- đầu. Chủ dự án chốt: **cộng thêm vào số hiện có** → 60.000. Giữ được cả hai
-- nguyên tắc — không thiếu hàng, và khoa quyết cuối.
-- ───────────────────────────────────────────────────────────────────────────
create or replace function xac_nhan_rot_v3(
    p_dot_goi_id bigint, p_giai_doan text default null, p_ma_hang text default null)
returns jsonb
...
-- nhảy sang gói bổ sung kế tiếp (fn_dot_bo_sung_gan_nhat đã làm đúng việc này).
-- KHÔNG tự trừ bớt của khoa nào — khoa quyết số cuối cùng (D11).
--
